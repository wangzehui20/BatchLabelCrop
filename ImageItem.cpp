...
QPointF ImageItem::getPolygonCenter(QPolygonF polygon) {
	...
	if ((x_maxValue - x_minValue) <= clipSize && (y_maxValue - y_minValue) <= clipSize){
...
void ImageItem::clipImage(QString dstDir, QString imgPath, QImage img, QVector<QPointF> centers) {
	QFileInfo imgInfo(imgPath);
	QString imgName = imgInfo.baseName();
	int width = img.width();
